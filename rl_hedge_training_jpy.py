...
Produces:
    ...
    optuna_best_w*.json (per-window best hyperparameters)

Key differences from rl_hedge_training.py (USD/ZAR, 2 hedge times):
    - THREE trade windows per day: 00 UTC (Tokyo open), 06 UTC (Tokyo PM), 16 UTC
    - Transaction costs differ by window (bps):
        00 UTC : 1.6  (Tokyo open — tightest spreads in JPY)
        06 UTC : 1.8  (Tokyo afternoon / London pre-open)
        16 UTC : 1.8  (London afternoon / NY overlap)
    - ACTION_MAP has 9 actions covering single-venue, split, and triple-split strategies
    - Observation and feature names updated to h00/h06/h16
    - Benchmark: always-hedge-at-16-UTC (same convention as ZAR for comparability)
    - 11 walk-forward windows (test years 2015–2025) vs ZAR's 6

Walk-forward structure with JPY data 2009-11-01 to 2026-04-08:
    W1:  Train 2009-2013 → Val 2014 → Test 2015
    W2:  Train 2009-2014 → Val 2015 → Test 2016
    ...
    W11: Train 2009-2023 → Val 2024 → Test 2025
...
# JPY features: h00 = Tokyo open, h06 = Tokyo PM, h16 = London/NY
STATE_FEATURES = [
    # Vol surface
...
# ════════════════════════════════════════════════════════════════
# SECTION 2: ACTION SPACE  (3 trade windows)
# ════════════════════════════════════════════════════════════════
#
# f00 = fraction of delta mismatch to trade at 00 UTC (Tokyo open)
# f06 = fraction of REMAINING mismatch to trade at 06 UTC (Tokyo PM)
# f16 = fraction of REMAINING mismatch to trade at 16 UTC (London/NY)
#
# "Remaining mismatch" cascades: each time slot sees the mismatch
# after all earlier trades for that day are accounted for.
#
# Transaction costs: 1.6 bps at 00 UTC, 1.8 bps at 06 UTC and 16 UTC.
# The RL agent implicitly learns this cost asymmetry.
...
# ════════════════════════════════════════════════════════════════
# SECTION 3: HEDGING ENVIRONMENT  (3 intraday trade times)
# ════════════════════════════════════════════════════════════════

class FXHedgingEnv(gym.Env):
    """
    USD/JPY delta-hedging environment with three intraday trade windows.
    All data is pre-computed numpy arrays — no live GK pricing in step().
    """

    def __init__(self,
                 state_features,
                 # Portfolio Greeks at close (always needed)
                 delta_close, gamma_close, price_close,
                 # Delta at each hedge window (for mismatch computation)
                 delta_00, delta_06, delta_16,
                 # Spot at each hedge window (for P&L computation)
                 spot_00, spot_06, spot_16, spot_close,
                 # Optional pre-computed continuity P&L
                 continuity_pnl=None,
        ...
        # Observation: state features + [hedge_pos, steps_since_hedge, delta_close, gamma_close]
        n_extra = 4
    ...
    def step(self, action):
        ...
        # ── Hedge P&L: carry + each trade earns from execution to close ─
        ...
        # ── Transaction costs (JPY-specific per-window rates) ──────────
        ...
        # ── Reward: log1p(squared hedge error) + TC penalty ────────────
        ...
        info = {
            'daily_pnl': daily_pnl, 'hedge_error': hedge_error,
            'option_pnl': option_pnl,
            'carry_pnl': carry_pnl,
            'trade00_pnl': trade00_pnl, 'trade06_pnl': trade06_pnl, 'trade16_pnl': trade16_pnl,
            'hedge_pnl': hedge_pnl, 'tc': tc,
            'tc00': tc00, 'tc06': tc06, 'tc16': tc16,
            'dH00': dH00, 'dH06': dH06, 'dH16': dH16,
            'delta_00': d00, 'delta_06': d06, 'delta_16': d16,
            'delta_close': self.delta_close[i],
            'spot_00': s00, 'spot_06': s06, 'spot_16': s16, 'spot_close': sc,
...
def run_benchmark(delta_16, price_close, spot_16, spot_close,
    ...
    Uses 16 UTC TC (1.8 bps). Same convention as ZAR benchmark for comparability.
    """
    ...
    for i in range(n):
        ...
        records.append({'daily_pnl': daily_pnl, 'hedge_error': hedge_error,
                        'carry_pnl': carry_pnl, 'trade_pnl': trade_pnl,
                        'tc': tc, 'trade_size': dH, 'delta_close': delta_16[i]})
...
def make_env(feat_df, dates, normalizer, feature_cols,
             reward_scaling=1000.0):
    f     = feat_df.loc[dates]
...
def build_model(env, params, use_recurrent):
    ...
    if use_recurrent:
        ...
        return PPO(
            ...
            gae_lambda     = params.get('gae_lambda', 0.95),
            clip_range     = params.get('clip_range', 0.2),
            ent_coef       = params.get('ent_coef', 0.025),
            vf_coef        = 0.5,
            max_grad_norm  = 0.5,
...
# ent_coef set to 0.05 (higher than ZAR default) because JPY vol is lower
# (~8-15% vs ZAR 15-25%) → greater entropy collapse risk in low-vol regimes.
DEFAULT_PARAMS = {
    'learning_rate':    3e-4,
    'ent_coef':         0.05,
    'clip_range':       0.2,
...
def tune_window(feat_df, train_dates, val_dates, feature_cols,
    ...
    val_bm   = run_benchmark(vf['port_delta_16'].values, vf['port_price_close'].values,
                              vf['spot_16'].values, vf['spot_close'].values, val_cont)
    bm_pnl   = val_bm['daily_pnl'].values
...
def analyse_actions(rl, feat_df, data_dir):
    ...
    # Classify action by primary venue (6-action map: 0=h100_00, 1=h75_00, 2=h100_06,
    #                                    3=split_50_00_16, 4=split_50_06_16, 5=h100_16)
    rl['is_00utc'] = rl['action'].isin([0, 1]).astype(int)      # pure 00 UTC
    rl['is_06utc'] = rl['action'].isin([2]).astype(int)          # pure 06 UTC
    rl['is_16utc'] = rl['action'].isin([5]).astype(int)          # pure 16 UTC
    rl['is_split'] = rl['action'].isin([3, 4]).astype(int)       # any split
    ...
    for split in splits:
        ...
        if os.path.exists(model_path + '.zip') and os.path.exists(metrics_path):
            ...
            test_bm = run_benchmark(tf['port_delta_16'].values, tf['port_price_close'].values,
                                     tf['spot_16'].values, tf['spot_close'].values, test_cont)
        ...
        norm = FeatureNormalizer(252)
        norm.fit(feat_df.loc[split['tr'], feature_cols])
        rs = best_params.get('reward_scaling', 1000.0)
        ...
        acts_norm = test_rl['action'].value_counts(normalize=True)
        logprint(f"    Policy entropy: {calc_entropy(acts_norm):.3f} "
...
def plot_results(data_dir='.'):
    ...
    ax.set_title('USD/JPY Out-of-Sample: RL Agent vs Benchmark (always hedge at 16 UTC)')
